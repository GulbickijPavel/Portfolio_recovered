...
//}
    if (isset($_GET['sectionname'])) {
        ...
        //    echo $comma;
          ...
          //  echo "UPDATE translations set $set where translationid $id";
            $sql = "UPDATE translations set $set where translationid = $id";
            $result = mysqli_query($link, $sql) or die(mysql_error($link));
            if ($result) {
                echo "Lentelė translations sekmingai atnaujinta.";
            }
    ...
    if(isset($_POST['sectionname']))
    {
        ...
        if($sectionname == 'delete'){
            ...
            if(!$result){
                ...
                echo "Įrašas sekmingai pašalintas.";
            }
        }
        ...
        }else if ($sectionname == 'full-section-remove') {
            $id = $_POST['itemid'];
            $table = $_POST['table'];
            $key = $_POST['key'];
            $query0 = "SELECT name FROM sections WHERE sectionid = $id";
            $result0 = mysqli_query($link, $query0) or die(mysqli_error($link));
            $file = mysqli_fetch_assoc($result0);
            $comma = implode(",", (array)$file);
            $file = $comma.'.tpl';
            $sql = "DELETE FROM $table where $key = $id";
            $result = mysqli_query($link, $sql) or die(mysqli_error($link));
            $query = "ALTER TABLE $table AUTO_INCREMENT = 1";
            ...
            if(!$result){
                ...
                echo "Įrašas ir šablonas sekmingai pašalinti.";
                unlink($path.'/smarty/templates/sections/'.$file);
            }
        }
    }
...
//        }
        ...
        // if everything is ok, try to upload file
            ...
            if (move_uploaded_file($file["tmp_name"], $target_file)) {
                ...
                echo "Įvyko klaida, nuotraka neįkelta.";
            }
        }
    }
